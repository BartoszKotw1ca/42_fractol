...
/*   Updated: 2024/04/23 10:38:25 by bkotwica         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_hook_mandelbrot(int keycode, int x, int y, t_fract *mlx)
{
	...
	mlx->mouse_y = y;
	if (keycode == 4)
		destory_and_put_mandel(mlx, x, y, 1.5);
	else if (keycode == 5)
		destory_and_put_mandel(mlx, x, y, 0.5);
	return (0);
}
...
int	key_hook_mandel(int keycode, t_fract *mlx)
{
	...
	if (keycode == 65307)
		...
		destory_and_put_mandel(mlx, mlx->center_x -= 10, mlx->center_y, 1);
	else if (keycode == 97)
		destory_and_put_mandel(mlx, mlx->center_x += 10, mlx->center_y, 1);
	else if (keycode == 115)
		destory_and_put_mandel(mlx, mlx->center_x, mlx->center_y -= 10, 1);
	else if (keycode == 119)
		destory_and_put_mandel(mlx, mlx->center_x, mlx->center_y += 10, 1);
	return (0);
}
